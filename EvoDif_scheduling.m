function [mejorindividuo, nfeval] = EvoDif(NP,J,orden,generaciones)
%Minimizaci贸n con Evoluci贸n diferencial
%Salidas
%--------------------
%bestmem	: mejor soluci贸n
%nfeval		: n煤mero de evaluaci贸n de la funci贸n

%Entradas
%--------------------
%NP		: N煤mero de individuos en la poblaci贸n
%M		: N鷐ero de m醧uinas (y operaciones por trabajo)
%N		: N鷐ero de trabajos
%D		: M*N
%J      : Matriz de trabajos
%generaciones	: m谩ximo n煤mero de generaciones
[M,N]=size(J);
count=(1:(M*N))';
%-----Inicializar poblaci贸n------------------------------ 
poblacion = zeros(M*N, NP);
%Cada columna de la poblaci髇 es una matriz de trabajos
for i=1:NP
	poblacion(:,i)=permutarTrabajos(J(:),M,N,orden);
end
poblacion_vieja		= zeros(size(poblacion));
val                 = zeros(NP,1);
mejorindividuo		= zeros(M*N,1);
mejorinditeracion	= zeros(M*N,1);
nfeval			=0;

%----Evaluaci贸n------------------------------------------
imejor	= 1; %铆ndice del mejor
val(1)	= costoSchedule(poblacion(:,imejor),M,N);
mejorval= val(1);
nfeval	= nfeval+1;
for i=2:NP
	val(i)	= costoSchedule(poblacion(:,i),M,N);
	nfeval	= nfeval+1;
	if (val(i)<mejorval)
		imejor	= i;
		mejorval= val(i);
	end
end
mejorinditeracion=poblacion(:,imejor);
mejorvaliteracion=mejorval;

mejorindividuo	= mejorinditeracion;
%----Minimizaci贸n---------------------------------------
%----Matrices de poblaciones---------------------------
mp1	=	zeros(M*N,NP);
mp2	=	zeros(M*N,NP);
% mp3	=	zeros(M*N,NP);
% mp4	=	zeros(M*N,NP);
% mp5	=	zeros(M*N,NP);
mi	=	zeros(M*N,NP);%Matriz de mejores individuos
ui	=	zeros(M*N,NP);%Vectores perturbados
% mui	=	zeros(NP,D);%"m谩scara" para la cruza
% mpo	=	zeros(NP,D);%"m谩scara" para la cruza
rot	=	(0:1:NP-1); %
rt	=	zeros(NP);
a1	=	zeros(NP);
a2	=	zeros(NP);
% a3	=	zeros(NP);
% a4	=	zeros(NP);
% a5	=	zeros(NP);
ind	=	zeros(4);

generacion=1;

while((generacion < generaciones) && (mejorval>1.e-5))
%    generacion
	poblacion_vieja = poblacion;
	%"barajar" poblaci贸n
	ind	=	randperm(4);
	a1	=	randperm(NP);
	rt	=	rem(rot+ind(1),NP);
	a2	=	a1(rt+1);
% 	rt	=	rem(rot+ind(2),NP);
% 	a3	=	a2(rt+1);
% 	rt	=	rem(rot+ind(3),NP);
% 	a4	=	a3(rt+1);
% 	rt	=	rem(rot+ind(4),NP);
% 	a5	=	a4(rt+1);

	mp1	=	poblacion_vieja(:,a1);
	mp2	=	poblacion_vieja(:,a2);
% 	mp3	=	poblacion_vieja(:,a3);
% 	mp4	=	poblacion_vieja(:,a4);
% 	mp5	=	poblacion_vieja(:,a5);

	for i=1:NP
		mi(:,i)=mejorinditeracion;
	end

	%mui	=	rand(NP,D) < CR;%"M谩scara" de los perturbados
	%mpv	=	mui < 0.5;	%铆dem poblaci贸n vieja
    
%-----Elegir la estrategia----------------------
%----------Mutaci贸n/Perturbaci贸n----------------
    for i=1:NP
        %offsprings
        ui(:,i)=CruzayMutacion(mp1(:,i),mp2(:,i),M,N,NP,orden);
    end
% 	if	(estrategia == 1)	% DE/best/1
% 		ui = mi+F*(mp1-mp2); 
% 	elseif	(estrategia == 2)	% DE/rand/1
% 		ui = mp3 + F*(mp1-mp2);
% 	elseif	(estrategia == 3)	% DE/rand-to-best/1
% 		ui = poblacion_vieja+F*(mi-poblacion_vieja) + F*(mp1-mp2);
% 	elseif	(estrategia == 4)	% DE/best/2
% 		ui = mi + F*(mp1-mp2 + mp3-mp4);
% 	else				% DE/rand/2
% 		ui = mp5 + F*(mp1-mp2 + mp3-mp4);
% 	end
%-----Cruza binomial-----------------------------
%	ui = poblacion_vieja.*mpv + ui.*mui;

%-----Elecci贸n de vectores de la nueva poblaci贸n---
	for i=1:NP
		val_tmp	=	costoSchedule(ui(:,i),M,N);
		nfeval	=	nfeval+1;
		if (val_tmp <= val(i))
			poblacion(:,i)	= ui(:,i);
			val(i)		= val_tmp;
			if (val_tmp < mejorval)
				mejorval 	= val_tmp;
				mejorindividuo	= ui(:,i);
			end
		end
	end
	mejorinditeracion	= mejorindividuo;
%-----Salida---------------------------------------------
 	if (rem(generacion,5)==0)
 		fprintf(1,'Iteracion: %d, Mejor: %f, NP %d\n',generacion, mejorval, NP);
 		%for n=1:(M*N)
 		%	fprintf(1,'Mejor(%d) = %f\n\n', n,mejorindividuo(n));
 		%end
 	end
%-----Plot-----------------------------------------------------------
generacion=generacion+1;
end%endwhile (generacion < generaciones)
mejorindividuo=mejorindividuo(:);
%mejorval=costoSchedule(mejorindividuo,M,N);
mejorindividuo=reshape(mejorindividuo,M,N);

end
